...
long read_line (void) {
  ...
  /*
  **   get initial delta-t from control database
  */
...
char *DATA_read_init (void) {
  ...
  */
  for (i = 0; i < num_data_files; i++) {
	...
	(void)snprintf (
	        buf, 256,
		"DATA_read_init - Spacing fwd to data - Check format of file %s.",
		fname[i]
	      );
	return (buf);
      }
  ...
  for (i = 1; i < num_data_files; i++) {
	...
	if (fd_ptr->time.jt == cur_fd->time.jt) {
	  (void)fprintf (
		  stderr,
		  "FILE_with_next_ts: "
		  "The files %s and %s both seem to contain the same storm on "
		  "%ld - %ld - %ld.\n",
...
char * EXTRACT_time (FILE_DATA *data) {
  ...
  if (data->time.year < 1800 || data->time.year > 2200) {
    (void)snprintf (err_buf, max_data_ln_len,
		    "EXTRACT_time - year %ld out of range.\nline:%s",
  ...
  if (errno == EDOM || errno == ERANGE) {
    (void)snprintf(err_buf, max_data_ln_len,
		   "EXTRACT_time - Decoding year line %s", start_point);
    return (err_buf);
  }
  ...
  if (data->time.month < 1 || data->time.month > 12) {
    (void)snprintf (err_buf, max_data_ln_len,
		    "EXTRACT_time - month %ld out of range.\nline:%s",
  ...
  if (errno == EDOM || errno == ERANGE) {
    (void)snprintf(err_buf, max_data_ln_len,
		   "EXTRACT_time - Decoding month line %s",start_point);
    return (err_buf);
  }
  ...
  if (data->time.day < 1 || data->time.day > 31) {
    (void)snprintf (err_buf, max_data_ln_len,
		    "EXTRACT_time - day %ld out of range.\nline:%s",
		    data->time.day, data->line);
    return (err_buf);
  }
  ...
  if (data->time.hour < 0 || data->time.hour > 24) {
    (void)snprintf(err_buf, max_data_ln_len,
		   "EXTRACT_time - hour %ld out of range.\nline:%s",
		   data->time.hour, data->line);
    return  (err_buf);
  }
  ...
  if (data->time.min < 0 || data->time.min > 59) {
    (void)snprintf (err_buf, max_data_ln_len,
		    "EXTRACT_time - minute %ld out of range.\nline:%s",
  ...
  if (data->time.sec < 0 || data->time.min > 59) {
    (void)snprintf (err_buf, max_data_ln_len,
		    "EXTRACT_time - second %ld out of range.\nline:%s",
