 ...
 *            store the values. This routine mainly handles the error
 *            conditions. Examples of legal strings for this routine
 *            are given in str_to_vals.c.
 */
...
long load_param (PARAM *param) {
  ...
  case M_DOUBLE:
	...
	(void)fprintf(
		stderr,
		"Assigned minimum value = %lf, "
		"Specified maximum value = %lf\n",
		dmin[i],
		dmax[i]
	      );
	return(1);
	...
	(void)fprintf(
		stderr,
		"\nERROR: assigned value is out of range for Parameter: "
		"'%s'\n", param->key
	      );
	(void)fprintf(
		stderr,
