...
/*--------------------------------------------------------------------*\
 ...
 | RETURN VALUE : None
\*--------------------------------------------------------------------*/
CONTROL *add_control (LIST *cont_db, char *key, long type, long size) {
   CONTROL *cp;
...
void decl_control (LIST *cont_db, char *key, long type, long size,
		   void *valstr) {
   CONTROL *cp;
...
void decl_control_string (LIST *cont_db, char *key, char *valstr) {
   char **cp = (char **)umalloc (sizeof (char *) * 1);
   
   *cp = strdup (valstr);
   decl_control (cont_db, strdup (key), M_STRING, 1, cp);
}

void decl_control_string_array (LIST *cont_db, char *key, long size, char *valstr) {
   char **cp;
   int i;
   ...
   decl_control (cont_db, strdup (key), M_STRING, size, cp);
}

void decl_control_int_array (LIST *cont_db, char *key, long size, long *valstr) {
   long *lp;
   int i;
   ...
   decl_control (cont_db, strdup (key), M_LONG, size, lp);
}

void decl_control_float_array (LIST *cont_db, char *key, long size, float *valstr) {
   float *fp;
   int i;
   ...
   decl_control (cont_db, strdup (key), M_FLOAT, size, fp);
}

void decl_control_double_array (LIST *cont_db, char *key, long size, double *valstr) {
   double *fp;
   int i;
   ...
   decl_control (cont_db, strdup (key), M_DOUBLE, size, fp);
}
...
/*--------------------------------------------------------------------*\
 ...
 | RESTRICTIONS :
\*--------------------------------------------------------------------*/
void decl_control_ (LIST *cont_db, char *ckey, ftnint *ctype, ftnint *csize, void *value, ftnlen klen) {
	char *key;
	long type, size;
   ...
   */
	...
	decl_control(cont_db, key, type, size, value);
	return;
}
