...
static char *rp (int index, int map_flag) {
  ...
  get_next_line ();
  if (index == 0) {
    /* if index equals zero, than this parameter file has dimension
       stuff and we need to skip over it. */
    while (strncmp (line, "** Parameters **", 16)) {
      if (!(line_p = get_next_line ())) {
	close_parameter_file();
      }
  ...
  */
  while (!feof (param_file)) {
	...
	buf_ptr = READ_param_values (param->size, param->type, param->name,
				     param->value, line, param->min_string,
				     param->max_string, param->bound_status);
      } else {

	/* Make sure that this resizing code is in sync with the
	   ParamToolExpandor code in the oui4 code base. */
	...
	buf_ptr = READ_param_values (param->pf_size, param->type, param->name,
				     pf_value, line, param->min_string,
				     param->max_string, param->bound_status);

	/* The values read from the parameter file need to be resized
	   to fit into the size of the module array for this
	   parameter. */
  ...
  if (*param_ptr) {
    ...
    */
	...
	return error_string("dimensions are incompatable with declaration in module");
      ...
      if (map_flag) {
	/* Need to set some values in the param structure for mapping
	   parameter */
	(*param_ptr)->size = (*param_ptr)->pf_size;
	// Mapping parameters are always integers
	(*param_ptr)->value = (char *)umalloc ((*param_ptr)->size * sizeof (int));
      }
      /*
       * get param size
       */
...
static int checkForValidDimensions (PARAM *param_ptr) {
  ...
  if (param_ptr->pf_ndimen > param_ptr->ndimen ) {
    /* more dimensions in the parameter file is always invalid */
    return 1;
    ...
    /* less dimensions in the parameter file than declared in the
       module. */
    badFlag = 1;
    for (i = 0; i < param_ptr->ndimen; i++) {
      /* check each dimension for compatiblilty; only need to find one
	 that is compatable */
      if (badFlag == 1) {
	// 0 = good;  1 = bad
	badFlag = isDimensionIncompatable (param_ptr->pf_dimNames[0],
					   param_ptr->dimen[i]->name);
      }
    }
    if (badFlag == 1) {
      return 1;
...
static int isDimensionIncompatable (char *pfDimName, char *modDimName) {
  ...
  if (!strncmp (pfDimName, modDimName, 10)) {
    // a dimension is compatable with itself
    return 0; 
  }

  if (!strncmp (pfDimName, "one", 3)) {
    // "one" in the parameter file is compatable with everything
    return 0; 
  }
...
static int getParamFileParamSize (PARAM *param) {
  ...
  size = 1;
  for (i = 0; i < param->pf_ndimen; i++) {
    // check each dimension for size
...
static char *get_next_line () {
  ...
  /* DANGER -- could have overflowed if max_data_ln_len was smaller
               than MAXDATALNLEN */
  if (line == NULL) {
...
static char *error_string (char *message) {
  static char buf[256];
  snprintf (buf, 256, "ERROR: %s; file is %s; line number %d",
	    message, file_name, lineNumber);
  return buf;
}
...
static char *warning_string (char *message) {
  static char buf[256];
  snprintf (buf, 256, "WARNING: %s; file is %s; line number %d",
	    message, file_name, lineNumber);
  return buf;
}
...
/*--------------------------------------------------------------------*\
  ...
  | COMMENT	: Generates the warning message when a long value is
  |               out of range.
\*--------------------------------------------------------------------*/
static void bad_param_value_l (long l, int i, char *name, long min_l, long max_l) {
  ...
  if (*control_lvar("parameter_check_flag") > 0) {
    snprintf (buf, 256, "%s[%d] = %d is out of range (%d to %d)",
	      name, i, (int)l, (int)min_l, (int)max_l);
    fprintf (stderr, "%s\n", warning_string(buf));
  }
}
...
/*--------------------------------------------------------------------*\
  ...
  | COMMENT	: Generates the warning message when a double or float
...
static void bad_param_value (double d, int i, char *name, double min_d, double max_d) {
  ...
  if (*control_lvar("parameter_check_flag") > 0) {
    snprintf (buf, 256, "%s[%d] = %f is out of range (%f to %f)",
	      name, i, d, min_d, max_d);
    fprintf (stderr, "%s\n", warning_string(buf));
  }
}
