...
/*--------------------------------------------------------------------*\
  ...
  | COMMENT	 : called from Fortran, sorts out args and calls
                   getparam()
\*--------------------------------------------------------------------*/
long getparam_ (char *mname, char *pname, ftnint *pmaxsize, char *ptype,
		double *pval,
...
/*--------------------------------------------------------------------*\
  ...
  \*--------------------------------------------------------------------*/
long getparam (char *module, char *name, int maxsize, char *type,
	       double *pval) {
  int var_type;
  PARAM *param;
  char *pkey;
  ...
  if((var_type != M_LONG) && (var_type != M_FLOAT) &&
     (var_type != M_DOUBLE) && (var_type != M_STRING)) {
    (void)fprintf(
	    stderr,
	    "\nERROR: data type for parameter "
	    "%s in module %s has inconsistent uses.\n", pkey, module
	  );
    return(1);
  }
  ...
  if (param == NULL) {
    (void)fprintf(
	    ...
	    "%s in module %s, but parameter is not found.\n", pkey, module
	  );
    return(1);
  }
  ...
  if (param->read_in == 0) {
    (void)fprintf(
	    stderr,
	    "\nWARNING: parameter "
	    "%s is used by module %s but values are not\n", pkey, module
	  );
    (void)fprintf(
	    stderr,
	    "         set in the Parameter File. "
	    "Module default values are being used.\n"
	  );
  }
  ...
  if (param->size > maxsize) {
    (void)fprintf(
	    stderr,
	    "\nERROR: parameter "
	    "%s declared array size is not big enough in module %s.\n",
	    pkey, module
	  );
    return(1);
  }
...
static long paramcopy (PARAM *param, double *pval, int maxsize) {
  ...
  if (param->ndimen == 1) {
	...
	memcpy ((char *)pval + i, *((char **)param->value+i),
		16 * sizeof(char *));
      }
      break;
    }
    ...
    for (i = 0; i < val2; i++) {
      ...
      case M_LONG:
	memcpy (ptr, (char *)(param->value + i * val1 * sizeof(int)),
		val1 * sizeof(int));
	ptr += nrow * sizeof(int);
	break;

      case M_FLOAT:
	memcpy (ptr, (char *) (param->value + i * val1 * sizeof(float)),
		val1*sizeof(float));
	ptr += nrow * sizeof(float);
	break;

      case M_DOUBLE:
	memcpy (ptr, (char *) (param->value + i * val1 * sizeof(double)),
		val1 * sizeof(double));
	ptr +=  nrow * sizeof(double);
	break;

      case M_STRING:
	memcpy (ptr, (char *) (param->value + i * val1 * sizeof(char *)),
		val1 * sizeof(char *));
...
/*--------------------------------------------------------------------*\
  ...
  | COMMENT	 : called from Fortran
\*--------------------------------------------------------------------*/
long getparamstring_ (char *mname, char *pname, ftnint *pmaxsize,
		      char *ptype, ftnint *pindex, char *pstring,
		      ftnlen mnamelen, ftnlen pnamelen, ftnlen ptypelen,
		      ftnlen pslen) {
