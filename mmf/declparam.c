...
long declparam_u_ (char *mname, char *pname, char *pdimen, char *ptype,
		   char *pvalstr, char *minstr, char *maxstr, char *dstr,
		   char *hstr,
...
\*--------------------------------------------------------------------*/
long declparam_u (char *module, char *name, char *dimen, char *type,
		  char *value, char *minimum, char *maximum, char *descr,
		  char *help, char *units, char *var, long *update) {
  ...
  *update = 0;
  ...
  if (param == NULL) {
    /* Parameter has not been declared, do so now. Set up array for
       pointers to local arrays of values. */
    declparam (module, name, dimen, type, value, minimum, maximum, descr,
	       help, units);
    param = param_addr(name);
    param->num_references = 0;
    param->size_references = 100;
    param->references =
      (void **)umalloc (param->size_references * sizeof(void *));
  }
  ...
  */
  if (param->num_references >= param->size_references - 1) {
    param->size_references += 100;
    param->references =
      (void **) urealloc ((char *)(param->references),
...
long declparam_ (char *mname, char *pname, char *pdimen, char *ptype,
		 char *pvalstr, char *minstr, char *maxstr, char *dstr,
		 char *hstr, char *ustr,
		 ftnlen mnamelen, ftnlen pnamelen, ftnlen pdimenlen,
		 ftnlen ptypelen, ftnlen pvallen, ftnlen minlen,
		 ftnlen maxlen, ftnlen dlen, ftnlen hlen, ftnlen ulen) {
...
long declparam (char *module, char *name, char *dimen, char *type, char *value,
		char *minimum, char *maximum, char *descr, char *help,
		char *units) {
  ...
  */
   ...
   */
  if (dimen == NULL) {
    /* ...this is a mapping parameter declared by read_params and not
       any module. Most of the information is unknown so return. */
    sort_params();
    return(0);
  }
  ...
  if (param->ndimen > MAX_NDIMEN) {
    ...
    (void)fprintf(stderr,
		  "Attempt to use %ld dimensions - this is too many.\n",
		  param->ndimen);
  ...
  /*
   * Check to see if the parameter values are to be bounded by a
   * dimension. If so, set the bound and bound_dimen fields, and set
   * the min and max strings as applicable. If bounded, the 'minimum'
   * string is set to "bounded", and the 'maximum' string contains the
   * name of the bounding dimension.
   */
   ...
   * maximum and minimum values will be retained for use when the
   * dimension is set to a non-zero value.
   */
  if (!param->size)
    param->size = 1;
...
long declparam_p_ (char *mname, char *pname, char *pdimen, char *ptype,
		   char *pvalstr, char *minstr, char *maxstr, char *dstr,
		   char *hstr, char *ustr, char *val, ftnlen mnamelen,
		   ftnlen pnamelen, ftnlen pdimenlen, ftnlen ptypelen,
		   ftnlen pvallen, ftnlen minlen, ftnlen maxlen,
		   ftnlen dlen, ftnlen hlen, ftnlen ulen, ftnlen vallen) {
...
/*--------------------------------------------------------------------*\
  ...
  | COMMENT	: declparam is called from C
\*--------------------------------------------------------------------*/
long declparam_p (char *module, char *name, char *dimen, char *type,
		  char *value, char *minimum, char *maximum, char *descr,
		  char *help, char *units, char *var) {
  PARAM *param;

  /* If the -preprocess command line arguement is not set, don't allow
     declaration of any "preprocess parameters." */
  if (!preprocess_on) {
    return 0;
  }
...
/*--------------------------------------------------------------------*\
  ...
  | RESTRICTIONS : None
\*--------------------------------------------------------------------*/
static int CHECK_param_in_db (char *pkey, char *module, char *dimen,
			      int var_type, char *value, char *minimum,
			      char *maximum, char *descr, char *help,
			      char *units) {
  ...
  if (check_param) {
    ...
    (void)snprintf (
	    buf, 1024,
	    "The parameter %s has been declared inconsistently "
	    "in the modules %s and %s.",
	    pkey, module, check_param->module
	  );
    ...
    if (strcmp (dimen, dim_names)) {
      ...
      (void)snprintf (
	      buf1, 256,
	      " The dimensions have been declared as %s and %s.",
	      dimen, dim_names
	    );
      strcat (buf, buf1);
    }
    ...
    if (var_type != check_param->type) {
      ...
      (void)snprintf (
	      buf1, 256,
	      " The types have been declared as %s and %s.",
	      types[var_type], types[check_param->type]
	    );
      strcat (buf, buf1);
    }
    ...
    if (strcmp (value, check_param->value_string)) {
      ...
      (void)snprintf (
	      buf1, 256,
	      " The default values have been declared as %s and %s.",
	      value, check_param->value_string
	    );
      strcat (buf, buf1);
    }
    ...
    if (strcmp (minimum, check_param->min_string)) {
      ...
      (void)snprintf (
	      buf1, 256,
	      " The minimum value has been declared as %s and %s.",
	      minimum, check_param->min_string
	    );
      strcat (buf, buf1);
    }
    ...
    if (strcmp (maximum, check_param->max_string)) {
      ...
      (void)snprintf (
	      buf1, 256,
	      " The maximum value has been declared as %s and %s.",
	      maximum, check_param->max_string
	    );
      strcat (buf, buf1);
    }
    ...
    if (strcmp (descr, check_param->descr)) {
      ...
      (void)snprintf (
	      buf, 1024,
	      " The parameter description has been set as \"%s\"and \"%s.\"",
	      descr, check_param->descr
	    );
      strcat (buf, buf1);
    }
    ...
    if (strcmp (units, check_param->units)) {
      ...
      (void)snprintf (
	      buf1, 256,
	      " The units have been set as %s and %s.",
	      help, check_param->help
	    );
      strcat (buf, buf1);
    }
